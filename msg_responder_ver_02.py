...
class MsgResponder(Frame):
    ...
    def btnClickedPass(self):
        ...
        result = 1
        #get the qc_step_name and qc_step_description as well
        comment = "<comment>" + self.text_qc_results_actual.get("1.0", END) + "</comment>" 
        print str(result)
        print comment
        self.quit()
    ...
    def initUI(self):
        ...
        label_qc_desc = Label(label_frame_qc_desc, text=self.qc_step_description)
        ...
        label_frame_qc_results_expected = LabelFrame(pane_bottom, text="QC Expected Results")
        label_qc_results_expected = Label(label_frame_qc_results_expected, text=self.qc_step_expected_results, wraplength=500, justify=LEFT)
        ...
        self.text_qc_results_actual = Text(pane_bottom, height=14, width=15)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-name', '--qc_step_name', help = 'the QC/ALM step name', default = None)
    parser.add_argument('-desc', '--qc_step_description', help = 'the QC/ALM step description', default = None)
    parser.add_argument('-expected', '--qc_step_expected_results', help = 'the QC/ALM step expected results', default = None)
